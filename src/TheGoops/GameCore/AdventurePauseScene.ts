...
enum AdventureScenePauseList {
    RESUME,
    REPLAY,
    LVLMENU
}

class AdventurePauseScene extends GameScene implements IListener {
    ...
    constructor(game: Core.Game) {
        ...
        this._btnMainMenu = new Button(game.Content.loadImage("lvlmenuBtn.png"), 75, 320, 80, 80);
    }
    ...
    EventNotify(sub: any, message?: any): void {
        if (sub instanceof ButtonAction) {
            if ((<ButtonAction>sub).button == this._btnPlay) {
                this._selectedItem = AdventureScenePauseList.RESUME;
            } else if ((<ButtonAction>sub).button == this._btnReplay) {
                this._selectedItem = AdventureScenePauseList.REPLAY;
            } else if ((<ButtonAction>sub).button == this._btnMainMenu) {
                this._selectedItem = AdventureScenePauseList.LVLMENU;
            }
        }
    }
